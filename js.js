...
// ======================================================
    ...
    // ======================================================
// === AUTOMATA IDŐZÍTETT RECAP RENDSZER ===
// ======================================================

function checkAndShowRecap() {
    const today = new Date();
    const dayOfWeek = today.getDay(); // 0: Vas, 1: Hétfő, ...
    ...
    // PRIORITÁSOK: Éves > Havi > Heti

    // 1. ÉVES RECAP (Január 1-jén)
    if (month === 0 && dayOfMonth === 1) {
        recapType = 'yearly';
        periodName = `A ${today.getFullYear() - 1}-es Éved`;
    }
    // 2. HAVI RECAP (Minden hónap 1-jén, kivéve Jan 1)
    else if (dayOfMonth === 1) {
        recapType = 'monthly';
        const prevMonthName = new Date(today.getFullYear(), month - 1, 1).toLocaleString('hu-HU', { month: 'long' });
        periodName = `${prevMonthName.charAt(0).toUpperCase() + prevMonthName.slice(1)}i Recap`;
    }
    // 3. HETI RECAP (Minden Hétfőn)
    else if (dayOfWeek === 1) {
        recapType = 'weekly';
        periodName = "Heti Összefoglaló";
    }

    // Ha nincs recap nap, kilépünk (vagy teszteléshez kikommentezheted a feltételeket)
    if (!recapType) {
        // TESZT MÓD: Ha szeretnéd mindig látni a teszteléshez, vedd ki a komments jelet:
        // recapType = 'weekly'; periodName = "TESZT Recap"; 
        return; 
    }

    // Adatok betöltése és szűrése az időszak alapján
    const allBeers = JSON.parse(localStorage.getItem('beers')) || [];
    const filteredBeers = filterBeersByPeriod(allBeers, recapType);

    // Ha nincs adat az időszakban, ne mutassuk a gombot
    if (filteredBeers.length === 0) return;

    // Ha van adat, generáljuk le a statisztikát előre
    const statsData = calculateRecapStats(filteredBeers);
    if (!statsData) return;
    
    statsData.periodName = periodName; // Hozzáadjuk a nevet

    // Megjelenítjük a "Story Ring" gombot
    renderRecapNotification(statsData);
}

// Segédfüggvény: Dátum szerinti szűrés
function filterBeersByPeriod(beers, type) {
    const now = new Date();
    const oneDay = 24 * 60 * 60 * 1000;
    
    return beers.filter(beer => {
        if (!beer.date) return false;
        const beerDate = parseBeerDate(beer.date); // Feltételezve, hogy van ilyen függvényed
        if (!beerDate) return false;

        if (type === 'weekly') {
            // Elmúlt 7 nap (nem beleértve a mait, szigorúan az elmúlt hét)
            // VAGY: Múlt hétfőtől vasárnapig. Itt most: elmúlt 7 nap.
            const diffDays = Math.round(Math.abs((now - beerDate) / oneDay));
        ...
        if (type === 'monthly') {
            ...
            return beerDate.getMonth() === prevMonthDate.getMonth() && 
                   beerDate.getFullYear() === prevMonthDate.getFullYear();
        }
        
        if (type === 'yearly') {
            // Előző év
            return beerDate.getFullYear() === (now.getFullYear() - 1);
        }
        return false;
    });
}

// Segédfüggvény: Értesítő kirajzolása
function renderRecapNotification(statsData) {
    const container = document.getElementById('recap-notification-area');
    if (!container) return;

    container.innerHTML = `
        <div class="recap-story-ring" id="triggerRecapBtn">
            <div class="recap-story-avatar">
                📊
            </div>
            <div class="recap-story-label">${statsData.periodName}</div>
        </div>
    `;

    // Klikk esemény: Indítja a Story módot (a korábban megírt renderStoryMode-ot)
    document.getElementById('triggerRecapBtn').addEventListener('click', () => {
        
        // 1. Overlay megjelenítése (ha van ilyen container a HTML-ben, ha nincs, létrehozzuk dinamikusan)
        let storyOverlay = document.getElementById('storyOverlay');
        if (!storyOverlay) {
            storyOverlay = document.createElement('div');
            storyOverlay.id = 'storyOverlay';
            storyOverlay.className = 'recap-overlay active';
            document.body.appendChild(storyOverlay);
        } else {
            storyOverlay.classList.add('active');
            storyOverlay.style.display = 'flex'; // Biztos ami biztos
        }

        // 2. Story renderelése
        renderStoryMode(statsData, storyOverlay);
    });
}
