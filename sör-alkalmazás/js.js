...
// Egy központi függvény, ami kommunikál a backendünkkel
async function callSheetApi(action, payload, range = 'Sheet1!A:Z') {
    ...
    if (!response.ok) {
        throw new Error('Hiba a szerveroldali függvénnyel való kommunikáció során.');
    }
...
async function saveUserToGoogleSheets(userData) {
    try {
        ...
        // Az adatokat egy tömbbe rendezzük a `sheet.js`-ben elvárt formátum szerint
        const userRow = [userData.id, userData.name, userData.email, userData.registeredAt];
        await callSheetApi('APPEND_DATA', userRow, 'Felhasználók!A:D'); // Tegyük fel, hogy a felhasználókat egy "Felhasználók" nevű lapra mentjük
        showSuccess('Felhasználó sikeresen mentve a felhőbe!');
    } catch (error) {
        console.error('Hiba a felhasználó mentésekor:', error);
        showError('Hiba a felhasználói adatok mentésekor.');
    }
}
...
async function saveBeerToGoogleSheets(beerData) {
    try {
        ...
        const beerRow = [beerData.id, beerData.name, beerData.type, beerData.alcohol, beerData.price, beerData.stock, beerData.addedAt];
        await callSheetApi('APPEND_DATA', beerRow, 'Sörök!A:G'); // A söröket a "Sörök" lapra mentjük
        showSuccess('Sör sikeresen mentve a felhőbe!');
    } catch (error) {
        console.error('Hiba a sör mentésekor:', error);
        showError('Hiba a sör adatok mentésekor.');
    }
}

async function updateBeerInGoogleSheets(beerData) {
    console.log('Sör frissítése Google Sheets-ben:', beerData);
    // A frissítés bonyolultabb, ehhez ki kellene bővíteni a sheet.js-t egy 'UPDATE_DATA' akcióval.
    // Ez megkeresné az adott ID-jű sort és frissítené az értékeket.
    showNotification('A frissítés funkció még nincs implementálva a szerver oldalon.', 'info');
}

async function deleteBeerFromGoogleSheets(beerId) {
    console.log('Sör törlése Google Sheets-ből:', beerId);
    // A törlés is egy új, 'DELETE_DATA' akciót igényelne a sheet.js-ben.
    showNotification('A törlés funkció még nincs implementálva a szerver oldalon.', 'info');
}
