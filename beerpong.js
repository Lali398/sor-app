...
function renderSetup(el) {
    if (!BP.setup) {
        BP.setup = { cups: 6, selected: [], drawn: null };
    }
    const s = BP.setup;
    const players = BP.roster.players || [];
    ...
    el.innerHTML = `
        <div class="bp-section">
            ...
            <div class="bp-card">
                <h4 class="bp-card-title">2️⃣ Ki játszik? <span class="bp-muted">(${s.selected.length} kiválasztva)</span></h4>
                ${players.length === 0
            ...
            <button class="bp-btn bp-btn-big bp-btn-primary ${s.selected.length < 2 ? 'disabled' : ''}" data-bp="draw-teams">
                🎲 Csapatok kisorsolása
            </button>
            ${s.selected.length === 1 ? '<p class="bp-muted bp-center">Legalább 2 játékos kell!</p>' : ''}
        </div>
    `;
}
...
function drawTeams() {
    ...
    if (!s || s.selected.length < 2) {
        bpToast('Válassz ki legalább 2 játékost! 👥', 'error');
        return;
    }

    const shuffled = shuffle(s.selected);
    const teamA = [], teamB = [];
    shuffled.forEach((p, i) => (i % 2 === 0 ? teamA : teamB).push(p));
